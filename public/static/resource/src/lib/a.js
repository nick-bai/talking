    ...
    var MessageHandler = (function () {
        ...
        MessageHandler.prototype.onReceived = function (msg, pubAckItem) {
            ...
            if (message.messageType === RongIMLib.RongIMClient.MessageType["HandShakeResponseMessage"]) {
                var session = message.content.data, bool = false;
                RongIMLib.RongIMClient._memoryStore.custStore[message.targetId] = session;
                if (session.serviceType == RongIMLib.CustomerType.ONLY_ROBOT || session.serviceType == RongIMLib.CustomerType.ROBOT_FIRST) {
                    // TODO 业务逻辑
                    if (!session["robotWelcome"]) {
                        session["robotWelcome"] = RongIMLib.RongIMClient._memoryStore.custStore["robotWelcome"] || "您好，很高兴为您服务，请问有什么可以帮您的？";
                        bool = true;
                    }
                }
                else if (session.serviceType == RongIMLib.CustomerType.ONLY_HUMAN || session.serviceType == RongIMLib.CustomerType.HUMAN_FIRST) {
                    if (!session["humanWelcome"]) {
                        session["humanWelcome"] = RongIMLib.RongIMClient._memoryStore.custStore["humanWelcome"] || "您好，请问有什么可以帮您的？";
                        bool = true;
                    }
                    if (session.notAutoCha == "1") {
                        RongIMLib.RongIMClient.getInstance().switchToHumanMode("KEFU145760441681012", {
                            onSuccess: function () { console.log("switchOk");},
                            onError: function () { console.log("switcherr");}
                        });
                    }
                }
                if (bool) {
